...
public class SkillsEntityListener extends EntityListener {
    ...
    public void doDamage(EntityDamageByEntityEvent event){
		...
		if(dplayer != null){
        	...
        	for(int i = 36; i < 40; i++){
        		SkillsItemStack sit = new SkillsItemStack(inv.getItem(i));
    			...
    			if(askill > 0){
        			...
        			if(ahit > 0){
        				if(sit.getDamage() + ahit < SkillsProperties.getArmorDurability(id)){
        					sit.setDamage(sit.getDamage() + ahit);
        				}
        				else
        				{
        					inv.clear(i);
    	...
    	if(dmg > 0){
    		...
    		hp = ((LivingEntity)event.getEntity()).getHealth() - dmg;
	    	if(hp > 0){ 
	    		//Omfg... I'm reaaaly love hey0 some times...
	    		EntityLiving el = (EntityLiving)((CraftEntity)event.getEntity()).getHandle();
        		el.l.a(el, (byte)2);
        		((LivingEntity)event.getEntity()).setHealth(hp);
	    	}
	    	else
	    	{
	    		if(dplayer != null){
	    			SkillsPlayer.get(dplayer).stopTimer();
	    		}
	    		if(aplayer != null){
    ...
    public void onEntityExplode(EntityExplodeEvent event) {
    	...
    	while(itr.hasNext()){
    		...
    		if(block.getTypeId() == 51 || block.getTypeId() == 7 || block.getTypeId() == 0){
	    		...
	    		if(durability < 1){
	    	    	if(net.minecraft.server.Block.m[block.getTypeId()].a(rand) > 0){
	    	    		if(SkillsProperties.explosionDrop >= Math.random()){
	    	    			ItemStack drop = new ItemStack(net.minecraft.server.Block.m[block.getTypeId()].a(0, rand), net.minecraft.server.Block.m[block.getTypeId()].a(rand));
